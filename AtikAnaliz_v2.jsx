...
let _prices = Object.fromEntries(COINS_BASE.map(c=>[c.s,c.p]));
...
function scoreByIndicator(indicator, rsi, stoch, macdData, kdj, bb, closes) {
  ...
  if (indicator === "RSI") {
    ...
    const bbWidth = (lastBB.upper - lastBB.lower) / lastBB.mid;
...
// Generate full analysis — now takes interval + indicator
function analyzeCoins(interval = "15m", indicator = "STOCHRSI") {
  const ivcfg = IV_CFG[interval] || IV_CFG["15m"];

  return COINS_BASE.map(({s,p}) => {
    const drift = (Math.random()-.49)*0.025 * ivcfg.vol;
    _prices[s] = (_prices[s]||p) * (1 + drift*0.3);
    const price = _prices[s];
    const change24 = (Math.random()-.46)*8;

    // Generate candles with interval-specific volatility
...
const INDICATORS = ["STOCHRSI","RSI","MACD","KDJ","BB"];
...
export default function AtikAnaliz() {
  ...
  const log = useCallback((msg, type="info") => {
    setLogs(p => [{id:Date.now()+Math.random(),msg,type,time:new Date().toLocaleTimeString("tr-TR")},...p].slice(0,80));
  }, []);
  ...
  const runScan = useCallback(() => {
    const fresh = analyzeCoins(scanInterval, scanIndicator);
                  ...
                  <div style={{display:"flex",gap:4,marginBottom:8,overflowX:"auto"}}>
                    {["STOCHRSI","RSI","MACD","KDJ","BB"].map(ind=>(
                      <button key={ind} onClick={()=>{setIndicator(ind);snd("tick");}} style={{
                        flexShrink:0,padding:"4px 10px",borderRadius:6,border:`1px solid ${indicator===ind?"#0ecb8166":"#2b3158"}`,cursor:"pointer",
